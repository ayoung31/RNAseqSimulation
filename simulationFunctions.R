...
#'
#' @export
simulateData<-function(nS=3, K=2, B=1, g=10000, n=50, pK=NULL, pB=NULL,
  ...
  if(save_file){
    ...
    if(is.null(save_pref)){
      file_name=sprintf("%d_%d_%d_%f_%f_%f_%f",
                        nS,K,n,LFCg,pDEg,beta0[1],phi0[1])   # just the first elt of beta0 and phi0
  ...
  for(i in 1:nsims){
    ...
    #### Major Edit Simulate Data from Multiple Studies
    
    fit <- vector(mode = "list", length = nS)
    cts <- vector(mode = "list", length = nS)
    mu_mat <- vector(mode = "list", length = nS)
    sigma_mat <- vector(mode = "list", length = nS)
    
    for(j in 1:nS){
      fit[[j]] <- simulate_counts(K=K,B=B,n=n,g=g,
                           cls=cls,SF=SF,
      ...
      cts[[j]] = fit[[j]]$cts; mu_mat[[j]] = fit[[j]]$mu_mat; sigma_mat[[j]] = fit[[j]]$sigma_mat
    }
